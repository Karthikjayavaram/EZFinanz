...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    setError('');
    const res = await registerUser(data.name, data.email, data.phone, data.password);
    if (res.success) {
      if (res.data?.emailOtp) sessionStorage.setItem('pendingEmailOtp', res.data.emailOtp);
      if (res.data?.phoneOtp) sessionStorage.setItem('pendingPhoneOtp', res.data.phoneOtp);
      navigate('/verify');
    } else {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-xl shadow-slate-200/50">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-slate-600">
            Join EZFINANZ to apply for a loan
          </p>
        </div>
